...
export default function SupervisorDashboard() {
  ...
  useEffect(() => {
    // Fetch supervisor's email from /get-user-info route
    axiosPrivate
      .get("/get-user-info")
      .then((response) => {
        const userData = response.data;
        if (userData.email) {
          setSupervisorEmail(userData.email); // Set the supervisor's email
        }
      })
      .catch((error) => {
  ...
  useEffect(() => {
    // Ensure supervisorEmail is available before making other requests
    if (supervisorEmail) {
      console.log(supervisorEmail);
      axiosPrivate
        .get(`/supervisor-projects/${supervisorEmail}`)
       
        .then(async (response) => {
          // Fetch projects based on the supervisor's email
          ...
          setProjects(projectsWithLecturerInfo);
          setSelectedProject(projectsWithLecturerInfo[0]);
        })
        .catch((error) => {
